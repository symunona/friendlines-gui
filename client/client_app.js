(function(window, $, ko, SVG, moment, u, Cookie, svgPanZoom, tinycolor) {
    ...
    function VM() {
        ...
        this.status.subscribe(function() {
            self.statusColor(self.defaultStatusColor);
        });
    ...
    function loadSavedStates() {
        ...
        if (localStorage.getItem('friendlines') && localStorage.getItem('friendlines') != 'undefined') {
            ...
            vm.actualFilter(vm.filters().find(function(e) {
                return e.name() == loadedData.actualFilter.name
            }))
            vm.actualProcessor(window.processors.find(function(p) {
                return p.name == loadedData.actualProcessorName
            }))
            vm.disabledUsers = loadedData.disabledUsers || {}
            var user = vm.statUsers().find(function(u) {
                return u.name == vm.activeStatUser()
            })
    ...
    function initPan(element, actualStat) {
        var pan = svgPanZoom(element, {
            ...
            onZoom: function(newZoom) {
                ...
                if (!isNaN(newZoom))
                    Cookie.set('zoompan.zoom.' + actualStat, newZoom, {
                        expires: 9999
                    })
            },
            beforePan: function() {},
            onPan: function(newPan) {
                //console.log(newPan)
                if (!isNaN(newPan.x) && !isNaN(newPan.y))
                    Cookie.set('zoompan.pan.' + actualStat, newPan, {
                        expires: 9999
                    })
            }
        });
        var span = JSON.parse(Cookie.get('zoompan.pan.' + actualStat) || '{"x":0, "y":0}') || {
            x: 0,
            y: 0
        };
